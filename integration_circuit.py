...
def make_integration_circuit(mod,inp_mod,bg_mod):
    ...
    gIE_GABA = (1.3 * mod) * nS                  # Weight of inhibitory to excitatory synapses
    ...
    # Inputs
    nu_ext_1 = (2400 * bg_mod) * Hz				                 # Firing rate of external Poisson input to neurons in population D1
    nu_ext_2 = (2400 * bg_mod) * Hz				                 # Firing rate of external Poisson input to neurons in population D2
    nu_ext = (2400 * bg_mod) * Hz				                   # Firing rate of external Poisson input to neurons in population Dn and I
    stim_input_rate = (40 * inp_mod) * Hz        # Firing rate of the stimulus related Poisson input (to D1 and D2)                                              
    ...
    # Stimlus inputs (tpfeffer, 04/12/19)
    stim_inputE1 = PoissonGroup(N_D1, rates = stim_input_rate) 
    stim_inputE2 = PoissonGroup(N_D2, rates = stim_input_rate)
    ...
    extconnSE1 = IdentityConnection(stim_inputE1, decisionE1, 'gea', weight = gextE / gLeakE)
    extconnSE2 = IdentityConnection(stim_inputE2, decisionE2, 'gea', weight = gextE / gLeakE)
    extconnI = IdentityConnection(extinputI, decisionI, 'gea', weight = gextI / gLeakI)
    
		# Return the integration circuit
    groups = {'DE': decisionE, 'DI': decisionI, 'DX1': extinputE1, 'DX2': extinputE2, 'DX3': extinputE3, 'DXI': extinputI, 'DS1': stim_inputE1, 'DS2': stim_inputE2}
    subgroups = {'DE1': decisionE1, 'DE2': decisionE2, 'DE3': decisionE3}  
    connections = {'selfnmda': selfnmda,
                   'extconnE1': extconnE1, 'extconnE2': extconnE2, 'extconnE3': extconnE3, 'extconnI': extconnI, 'extconnSE1': extconnSE1, 'extconnSE2': extconnSE2,
